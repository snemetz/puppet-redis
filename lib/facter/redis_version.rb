...
Facter.add("redis_version", :timeout => 20) do
	...
	# OS independent
        redis_version = Facter::Util::Resolution.exec("/usr/sbin/redis-server --version | /bin/awk '{print $3}' | /bin/cut -d= -f2")
	# OS specific
        if redis_version.nil?
            redis_version = Facter::Util::Resolution.exec(yum+" info redis 2> /dev/null | /bin/grep '^Version' | /bin/cut -d: -f2'").strip
        end

	# Verify that version is of format #.#.# else return nil
        #case redis_version
        #    when /2\.8\.[0-9]/
        #        #set version to 2.8
        #        redis_version = '2.8.x'
